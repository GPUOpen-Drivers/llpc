...
.entry:
  ...
  %l = load <8 x i32>, ptr addrspace(4) %5, align 32, !invariant.load !10
  br label %loop

loop:
  %phi.ind = phi i32 [ 0, %.entry ], [ %ind, %loop ]
  %phi.load = phi <8 x i32> [ %l, %.entry ], [ %10, %loop ]
  ...
  %10 = load <8 x i32>, ptr addrspace(4) %8, align 32, !invariant.load !10
  %11 = call reassoc nnan nsz arcp contract afn <4 x float> (...) @lgc.create.image.sample.v4f32(i32 1, i32 24, <8 x i32> %phi.load, <4 x i32> %9, i32 1, <2 x float> zeroinitializer)
...
; CHECK-NEXT:    [[L:%.*]] = load <8 x i32>, ptr addrspace(4) [[TMP17]], align 32, !invariant.load !10
; CHECK-NEXT:    br label [[LOOP:%.*]]
; CHECK:       loop:
; CHECK-NEXT:    [[PHI_IND:%.*]] = phi i32 [ 0, [[DOTENTRY:%.*]] ], [ [[IND:%.*]], [[LOOP]] ]
; CHECK-NEXT:    [[PHI_LOAD:%.*]] = phi <8 x i32> [ [[L]], [[DOTENTRY]] ], [ [[TMP22:%.*]], [[LOOP]] ]
; CHECK-NEXT:    [[TMP18:%.*]] = mul i32 [[PHI_IND]], 48
...
; CHECK-NEXT:    [[TMP22]] = load <8 x i32>, ptr addrspace(4) [[TMP20]], align 32, !invariant.load !10
; CHECK-NEXT:    [[TMP23:%.*]] = call i32 @llvm.amdgcn.waterfall.begin.v8i32(i32 0, <8 x i32> [[PHI_LOAD]])
; CHECK-NEXT:    [[TMP24:%.*]] = call i32 @llvm.amdgcn.waterfall.begin.i32(i32 [[TMP23]], i32 [[TMP18]])
; CHECK-NEXT:    [[TMP25:%.*]] = call <8 x i32> @llvm.amdgcn.waterfall.readfirstlane.v8i32.v8i32(i32 [[TMP24]], <8 x i32> [[PHI_LOAD]])
; CHECK-NEXT:    [[TMP26:%.*]] = call i32 @llvm.amdgcn.waterfall.readfirstlane.i32.i32(i32 [[TMP24]], i32 [[TMP18]])
; CHECK-NEXT:    [[TMP27:%.*]] = sext i32 [[TMP26]] to i64
; CHECK-NEXT:    [[TMP28:%.*]] = getelementptr i8, ptr addrspace(4) [[TMP14]], i64 [[TMP27]]
; CHECK-NEXT:    [[TMP29:%.*]] = load <4 x i32>, ptr addrspace(4) [[TMP28]], align 16, !invariant.load !10
; CHECK-NEXT:    [[TMP30:%.*]] = call reassoc nnan nsz arcp contract afn <4 x float> @llvm.amdgcn.image.sample.2d.v4f32.f32(i32 15, float 0.000000e+00, float 0.000000e+00, <8 x i32> [[TMP25]], <4 x i32> [[TMP29]], i1 false, i32 0, i32 0)
...
; CHECK-NEXT:    call void @lgc.output.export.generic.i32.i32.v4f32(i32 0, i32 0, <4 x float> [[TMP31]]) #[[ATTR5:[0-9]+]]
