...
loop:
  ...
  %phi.img = phi <4 x float> [ <float 1.0, float 1.0, float 1.0, float 1.0>, %.entry ], [ %11, %loop ]
  ...
  %12 = fadd <4 x float> %phi.img, <float 1.0, float 1.0, float 1.0, float 1.0>
  %ind = add i32 %phi.ind, 1
  %cond = icmp ne i32 %ind, 1000
  br i1 %cond, label %loop, label %exit

exit:
  call void (...) @lgc.create.image.store(<4 x float> %12, i32 0, i32 8, <4 x i32> %9, i32 1)
  ret void
}
...
declare void @lgc.create.image.store(...) local_unnamed_addr #4
...
; CHECK-NEXT:    [[PHI_IMG:%.*]] = phi <4 x float> [ <float 1.000000e+00, float 1.000000e+00, float 1.000000e+00, float 1.000000e+00>, [[DOTENTRY]] ], [ [[TMP32:%.*]], [[LOOP]] ]
; CHECK-NEXT:    [[TMP15:%.*]] = mul i32 [[PHI_IND]], 48
...
; CHECK-NEXT:    [[TMP33:%.*]] = fadd <4 x float> [[PHI_IMG]], <float 1.000000e+00, float 1.000000e+00, float 1.000000e+00, float 1.000000e+00>
; CHECK-NEXT:    [[IND]] = add i32 [[PHI_IND]], 1
; CHECK-NEXT:    [[COND:%.*]] = icmp ne i32 [[IND]], 1000
; CHECK-NEXT:    br i1 [[COND]], label [[LOOP]], label [[EXIT:%.*]]
; CHECK:       exit:
; CHECK-NEXT:    [[TMP34:%.*]] = call i32 @llvm.amdgcn.waterfall.begin.i32(i32 0, i32 [[TMP18]])
; CHECK-NEXT:    [[TMP35:%.*]] = call i32 @llvm.amdgcn.waterfall.readfirstlane.i32.i32(i32 [[TMP34]], i32 [[TMP18]])
; CHECK-NEXT:    [[TMP36:%.*]] = sext i32 [[TMP35]] to i64
; CHECK-NEXT:    [[TMP37:%.*]] = getelementptr i8, ptr addrspace(4) [[TMP14]], i64 [[TMP36]]
; CHECK-NEXT:    [[TMP38:%.*]] = load <4 x i32>, ptr addrspace(4) [[TMP37]], align 16, !invariant.load !10
; CHECK-NEXT:    [[TMP39:%.*]] = call <4 x i32> @llvm.amdgcn.waterfall.last.use.v4i32(i32 [[TMP34]], <4 x i32> [[TMP38]])
; CHECK-NEXT:    call void @llvm.amdgcn.struct.buffer.store.format.v4f32(<4 x float> [[TMP33]], <4 x i32> [[TMP39]], i32 1, i32 0, i32 0, i32 0)
; CHECK-NEXT:    ret void
;
