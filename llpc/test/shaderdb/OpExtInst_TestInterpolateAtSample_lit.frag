...
; SHADERTEST: %{{[^ ]+}} = call {{.*}} float @interpolateAtSample.p64f32.i32(float addrspace(64)* @{{.*}}, i32 %{{[^) ]+}})
; SHADERTEST: %{{[^ ]+}} = call {{.*}} <4 x float> @interpolateAtSample.p64v4f32.i32(<4 x float> addrspace(64)* @{{.*}}, i32 %{{[^) ]+}})
...
; SHADERTEST: %{{[^ ]+}} = call float @llvm.amdgcn.interp.mov(i32 {{.*}}2, i32 immarg 1, i32 immarg 1, i32 %{{[^) ]+}})
