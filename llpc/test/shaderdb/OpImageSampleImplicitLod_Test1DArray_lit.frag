...
; SHADERTEST: call {{.*}} @lgc.create.image.sample.v4f32(i32 4, i32 0, {{.*}}, {{.*}}, i32 1, <2 x float> <float 1.000000e+00, float 2.000000e+00>)
...
; SHADERTEST: call {{.*}} <4 x float> @llvm.amdgcn.image.sample.1darray.v4f32.f32({{.*}}, float 1.000000e+00, float 2.000000e+00, {{.*}})
