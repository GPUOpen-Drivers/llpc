...
; SHADERTEST: %{{[^ ]+}} = call <2 x float> @lgc.input.import.builtin.InterpPerspCentroid(i32 {{.*}})
; SHADERTEST: %{{[^ ]+}} = call <4 x half> @lgc.input.import.interpolant.v4f16.i32.i32.i32.i32.v2f32(i32 0, i32 0, i32 0, i32 0, <2 x float> %{{[^) ]+}})
