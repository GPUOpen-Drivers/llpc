...
; RUN: amdllpc --llpc-opt=none    -spvgen-dir=%spvgendir% -v %gfxip %s | FileCheck --check-prefixes=SHADERTEST,OPT_QUICK %s
; RUN: amdllpc --llpc-opt=quick   -spvgen-dir=%spvgendir% -v %gfxip %s | FileCheck --check-prefixes=SHADERTEST,OPT_QUICK %s
; RUN: amdllpc --llpc-opt=default -spvgen-dir=%spvgendir% -v %gfxip %s | FileCheck --check-prefixes=SHADERTEST,OPT_DEFAULT %s
; RUN: amdllpc --llpc-opt=fast    -spvgen-dir=%spvgendir% -v %gfxip %s | FileCheck --check-prefixes=SHADERTEST,OPT_FAST %s

; Test that the LGC option overrides the amdllpc option.
; RUN: amdllpc --llpc-opt=fast --opt=none    -spvgen-dir=%spvgendir% -v %gfxip %s | FileCheck --check-prefixes=SHADERTEST,OPT_NONE %s
