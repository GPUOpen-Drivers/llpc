...
; RUN: opt --opaque-pointers=0 --enforce-pointer-metadata=1 --verify-each -passes='add-types-metadata,save-continuation-state,lint,remove-types-metadata' -S %s 2>%t.stderr | FileCheck %s
...
; CHECK-NEXT:    store i32 [[TMP0:%.*]], i32* [[CSP]], align 4
; CHECK-NEXT:    [[TMP3:%.*]] = load i32, i32* [[CSP]], align 4
; CHECK-NEXT:    [[TMP4:%.*]] = add i32 [[TMP3]], 0
; CHECK-NEXT:    store i32 [[TMP4]], i32* [[CSP]], align 4
; CHECK-NEXT:    [[TMP5:%.*]] = inttoptr i32 [[TMP3]] to i8 addrspace(21)*
; CHECK-NEXT:    [[FRAMEPTR:%.*]] = bitcast i8* addrspacecast (i8 addrspace(20)* bitcast ([0 x i32] addrspace(20)* @CONTINUATION_STATE to i8 addrspace(20)*) to i8*) to %simple_await.Frame*
; CHECK-NEXT:    [[RET_ADDR:%.*]] = load i64, i64 addrspace(20)* @RETURN_ADDR, align 8
; CHECK-NEXT:    [[DOTSPILL_ADDR:%.*]] = getelementptr inbounds [[SIMPLE_AWAIT_FRAME:%.*]], %simple_await.Frame* [[FRAMEPTR]], i32 0, i32 0
; CHECK-NEXT:    store i64 [[RET_ADDR]], i64* [[DOTSPILL_ADDR]], align 8
; CHECK-NEXT:    [[RESUME_FUNC:%.*]] = bitcast { i8*, %continuation.token* } (i8*, i1)* bitcast (void (i32)* @simple_await.resume.0 to { i8*, %continuation.token* } (i8*, i1)*) to i8*
; CHECK-NEXT:    [[TMP6:%.*]] = insertvalue { i8*, %continuation.token* } undef, i8* [[RESUME_FUNC]], 0
; CHECK-NEXT:    [[TMP7:%.*]] = ptrtoint i8* [[RESUME_FUNC]] to i64
; CHECK-NEXT:    store i64 [[TMP7]], i64 addrspace(20)* @RETURN_ADDR, align 8
; CHECK-NEXT:    call void (i64, ...) @continuation.continue(i64 ptrtoint (%continuation.token* ()* @async_fun to i64))
...
; CHECK-NEXT:    [[DOTRELOAD:%.*]] = load i64, i64* [[DOTRELOAD_ADDR]], align 8
; CHECK-NEXT:    [[TMP2:%.*]] = load i32, i32* [[CSP]], align 4
; CHECK-NEXT:    [[TMP3:%.*]] = add i32 [[TMP2]], 0
; CHECK-NEXT:    store i32 [[TMP3]], i32* [[CSP]], align 4
; CHECK-NEXT:    [[TMP4:%.*]] = inttoptr i32 [[TMP3]] to i8 addrspace(21)*
...
; CHECK-LABEL: @odd_cont_state_size(
; CHECK-NEXT:    [[CSP:%.*]] = alloca i32, align 4
; CHECK-NEXT:    store i32 [[TMP0:%.*]], i32* [[CSP]], align 4
; CHECK-NEXT:    [[TMP3:%.*]] = load i32, i32* [[CSP]], align 4
; CHECK-NEXT:    [[TMP4:%.*]] = add i32 [[TMP3]], 0
...
; CHECK-LABEL: @large_cont_state_size(
; CHECK-NEXT:    [[CSP:%.*]] = alloca i32, align 4
; CHECK-NEXT:    store i32 [[TMP0:%.*]], i32* [[CSP]], align 4
; CHECK-NEXT:    [[TMP3:%.*]] = load i32, i32* [[CSP]], align 4
; CHECK-NEXT:    [[TMP4:%.*]] = add i32 [[TMP3]], 0
