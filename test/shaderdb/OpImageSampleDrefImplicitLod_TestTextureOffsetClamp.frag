...
void main()
{
    ...
    fragColor.x += textureOffsetClampARB(samp1DShadow[index], vec3(0.1), 2, lodClamp);

    fragColor.x += textureOffsetClampARB(samp2DShadow, vec3(0.1), ivec2(2), lodClamp);

    fragColor.x += textureOffsetClampARB(samp1DArrayShadow, vec3(0.1), 2, lodClamp);

    fragColor.x += textureOffsetClampARB(samp2DArrayShadow, vec4(0.1), ivec2(2), lodClamp);
}
