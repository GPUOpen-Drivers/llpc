...
def parse_commandline_args(parser):
    ...
    # Generalize Call instructions to hide type suffixes
    parser.add_argument(
        "--generalize-calls",
        action="store_true",
        default=False,
        help="Generalize call instructions in IR by not checking for exact type suffixes.",
    )
...
def invoke_tool_only(exe, cmd_args, ir, verbose=False):
    ...
    if isinstance(cmd_args, list):
        ...
        stdout = subprocess.check_output(
            exe + " " + applySubstitutions(cmd_args, substitutions), shell=True
        )
...
# Generalize calls
TYPE_RE = r"(p\d+)?(v\d+)?([ifp]\d+|half|float|double|void|ptr)"
TYPE_SUFFIX_RE = r"([._]" + TYPE_RE + ")+"
CALL_INST_RE = re.compile(
    r"call " + r".*" + r" @.*" + r"(?P<types>" + TYPE_SUFFIX_RE + r")"
)
    ...
    *,
    ...
    if ginfo.is_ir():
        for i, line in enumerate(lines):
            ...
            if re.search(CALL_INST_RE, line) and generalize_calls:
                line = re.sub(re.compile(TYPE_SUFFIX_RE + r"\("), "{{.*}}(", line)
        ...
        for checkprefix in checkprefixes:
            ...
            if args_and_sig:
                args_and_sig = generalize_check_lines(
                    ...
                    generalize_calls=generalize_calls,
                )[0]
